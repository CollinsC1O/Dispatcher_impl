...
impl IERC20DispatcherImpl of IERC20DispatcherTrait<IERC20Dispatcher> {
    fn name(self: IERC20Dispatcher) -> felt252 {
        ...
        let mut dispatcher_return_data__ = starknet::syscalls::call_contract_syscall(
            self.contract_address, selector!("name"), core::array::ArrayTrait::span(@calldata),
        );

        let mut dispatcher_return_data__ = starknet::SyscallResultTrait::unwrap_syscall(
            dispatcher_return_data__,
        );

        core::option::OptionTrait::expect(
            core::serde::Serde::deserialize(ref dispatcher_return_data__), 'return value too short',
        )
    }
    ...
    fn transfer(self: IERC20Dispatcher, recipient: ContractAddress, amount: u256) {
        ...
        //Appending argumment into calldata aray
        core::serde::Serde::<ContractAddress>::serialize(@recipient, ref calldata_);
        core::serde::Serde::<u256>::serialize(@amount, ref calldata_);

        let mut dispatcher_return_data_ = starknet::SyscallResultTrait::unwrap_syscall(
            starknet::syscalls::call_contract_syscall(
                self.contract_address,
                selector!("transfer"),
